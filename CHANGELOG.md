...
- Default connection fallback uses `Broadcasting.default ?? 'null'` when the `default` connection is missing (PHP 8.5-safe; Laravel `#59269`)

### Changed

- Drop unused `firebase/php-jwt` requirement so Composer installs are not blocked by advisory PKSA-y2cr-5h3j-g3ys (CVE-2025-45769); the package was not used in plugin code
- Extract `handleBroadcasterBuildFailure()` from `_buildBroadcaster()` (behavior unchanged)

### Documentation

- Document CSRF skip for `/broadcasting/auth` and `/broadcasting/user-auth` (`skipCheckCallback` or scoped middleware) when CSRF is enabled globally
